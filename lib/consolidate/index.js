...
function retrieve(path, encoding, fn) {
...
function retrieveTemplate(path, options, fn) {
    ...
    // read
    retrieve(path, 'utf8', function(err, str){
        if (err) {
            return fn(err);
        }
...
function retrievePartials(path, options, fn) {
  if (!options.partials) {
      return fn();
  }
  ...
  function next(index) {
      ...
      retrieveTemplate(file, options, function(err, str){
          if (err) {
              return fn(err);
          }
...
function fromStringRenderer(name, call) {
  call = call || 'render';

  return function(path, options, fn){
      options.filename = path;

      retrievePartials(path, options, function (err) {
          if (err) {
              return fn(err);
          }

          if (cache(options)) {
              exports[name][call]('', options, fn);
          } else {
              read(path, options, function(err, str){
                  ...
                  exports[name][call](str, options, fn);
              });
          }
      });
  };
